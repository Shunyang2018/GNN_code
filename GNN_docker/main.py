...
def main(args):
    ...
    if not args['batch_mode']:
        ...
        combine(args["file_path"],df,args['smiles_column'],args['rt_column'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tp', '--column-type', type=str, default='c18',
                        help='Types of pretrained column specific model, which will be used for '
    ...
    parser.add_argument('-batch', '--batch-mode', type=bool, default=False,
                        help='whether use batch run to add RT fileter column to the original input csv file')
    parser.add_argument('-rc', '--rt-column', type=str,default='RetentionTime',
                        help='Header for the RT column in the CSV file, must be '
                             'provided if using batch mode')
    args = parser.parse_args().__dict__

    # Load configuration
    with open(args['column_type'] + '/configure.json', 'r') as f:
    ...
    if args['file_path'].endswith('.csv') or args['file_path'].endswith('.csv.gz'):
        ...
        if args['smiles_column'] is not None:
            smiles = df[args['smiles_column']][df[args['smiles_column']].notnull()].tolist()
        else:
    ...
    assert os.path.exists(args['column_type']), \
