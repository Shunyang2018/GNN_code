...
def calculate_retention_time_difference(data, column='C18RT'):
    """
    Calculate the difference in retention time for duplicate compounds.

    Parameters:
    data (DataFrame): Input dataframe containing the data.
    column (str): The column containing retention time values.

    Returns:
    DataFrame: DataFrame containing InChIKeys and their retention time differences.
    float: Mean of retention time differences.
    """
    # Filter rows where the specified column is not null
    filtered_data = data[data[column].notnull()]

    # Identify unique InChIKeys with duplicates
    duplicated_keys = filtered_data.loc[
        filtered_data.duplicated(['INCHIKEY'], keep=False), 'INCHIKEY'
    ].unique()

    # Calculate retention time differences for each duplicated key
    differences = [
        filtered_data.loc[filtered_data['INCHIKEY'] == key, column].max() -
        filtered_data.loc[filtered_data['INCHIKEY'] == key, column].min()
        for key in duplicated_keys
    ]

    # Create a DataFrame for the results
    results_df = pd.DataFrame({'INCHIKEY': duplicated_keys, 'diff': differences})

    # Calculate the mean difference
    mean_diff = results_df['diff'].mean()

    # Plot the distribution of differences
    sns.displot(differences, binwidth=0.1).set(xlabel=column)

    return results_df, mean_diff

import requests
import urllib.parse

def query(smiles_list, server_url):
    """
    Send synchronous requests to classify SMILES using a specified server.

    Args:
        smiles_list (list of str): List of SMILES strings to query.
        server_url (str): Base URL of the classification server.

    Returns:
        list: List of responses from the server.
    """
    all_urls = [
        f"{server_url}/classify?smiles={urllib.parse.quote(smiles)}"
        for smiles in smiles_list
    ]
    responses = [requests.get(url).text for url in all_urls]
    return responses
...
def smiles_to_inchikey(smiles):
    ...
    Returns:
        str: The corresponding InChIKey, or None if conversion fails.
    """
    try:
